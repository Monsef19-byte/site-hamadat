...
export default function NewArticlePage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // Generate a temp ID for the new article
    const newId = `new-${Date.now()}`;
    if (coverImage) {
      updateConfig({
        blog: { ...config.blog, [newId]: { coverImage } },
      });
    }
    setSaved(true);
            ...
            <div style={{ background: 'var(--bg-card)', borderRadius: '4px', padding: '28px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
              ...
              <div style={{ marginBottom: '16px' }}>
                ...
                <input type="text" required value={form.title}
                  ...
                  style={inputStyle('title')} placeholder="Titre de l'article" />
              </div>
              <div style={{ marginBottom: '16px' }}>
                <Label>Extrait</Label>
                <textarea required value={form.excerpt}
                  onChange={e => setForm({ ...form, excerpt: e.target.value })}
                  onFocus={() => setFocused('excerpt')} onBlur={() => setFocused(null)}
                  style={{ ...inputStyle('excerpt'), minHeight: '80px', resize: 'vertical' }}
              ...
              <div>
                ...
                <textarea value={form.content}
                  ...
                  style={{ ...inputStyle('content'), minHeight: '200px', resize: 'vertical' }}
            ...
            <div style={{ background: 'var(--bg-card)', borderRadius: '4px', padding: '28px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
              ...
              <div style={{ marginBottom: '16px' }} dir="rtl">
                ...
                <input type="text" value={form.titleAr}
                  ...
                  onFocus={() => setFocused('titleAr')} onBlur={() => setFocused(null)}
                  style={{ ...inputStyle('titleAr'), textAlign: 'right' }} placeholder="عنوان المقال" />
              </div>
              <div style={{ marginBottom: '16px' }} dir="rtl">
                <Label>ملخص</Label>
                <textarea value={form.excerptAr}
                  onChange={e => setForm({ ...form, excerptAr: e.target.value })}
                  onFocus={() => setFocused('excerptAr')} onBlur={() => setFocused(null)}
                  style={{ ...inputStyle('excerptAr'), minHeight: '80px', resize: 'vertical', textAlign: 'right' }} />
              </div>
            </div>
          </div>
              ...
              <div style={{ marginBottom: '16px' }}>
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }}>
                  <input type="checkbox" checked={form.published}
                    ...
                    style={{ width: '16px', height: '16px', accentColor: '#0e7470', cursor: 'pointer' }} />
